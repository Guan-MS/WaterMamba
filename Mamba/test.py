...
parser = argparse.ArgumentParser(description='Underwater Image Enhancement using WaterMamba')

parser.add_argument('--yaml', default='Mamba/Options/WaterMamba.yml', type=str, help='Path to yaml')
parser.add_argument('--input_dir', default='demo/input', type=str,
                    help='Directory of validation input images')
parser.add_argument('--target_dir', default='demo/target', type=str,
...
model_restoration = WaterMamba(**x['network_g'])
...
for dataset in datasets:
    ...
    with torch.no_grad():
        ...
        for file_ in tqdm(files):
            ...
            ssim_val = ssim(output, target_, data_range=1.1, size_average=False).item()
        ...
        print('final PSNR:{:.4f}'.format(np.mean(PSNR)), 'final SSIM:{:.4f}'.format(np.mean(SSIM)))
